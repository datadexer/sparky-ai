...
def validate_config(features, target, prices, model_name, params):
    ...
    for year in years:
        ...
        daily_signals = hourly_preds.resample('D').apply(
            lambda x: (x > 0.5).mean() >= 0.6
        ).astype(int)
...
def main():
    ...
    for i, cfg in enumerate(configs):
        print(f"[{i+1}/{len(configs)}] {cfg['model']} depth={cfg['params'].get('depth', cfg['params'].get('max_depth'))} lr={cfg['params']['learning_rate']}")
    ...
    for i, r in enumerate(all_results[:3], 1):
        print(f"{i}. {r['model']} (depth={r['params'].get('depth', r['params'].get('max_depth'))}, lr={r['params']['learning_rate']:.3f})")
