...
def walk_forward_static(
    prices_daily, daily_returns, signal_fn, label: str
) -> dict:
    """
...
def walk_forward_ml(
    prices_daily, daily_returns, df_daily, target_daily, feat_cols, label: str,
...
def run_lgbm_regime_combos(prices_daily, daily_returns, df_daily, target_daily, avail10, tracker):
    ...
    res_or = walk_forward_ml(
        prices_daily, daily_returns, df_daily, target_daily, avail10,
        "lgbm_vol_adx_OR",
...
def run_invvol_ensemble(prices_daily, daily_returns, tracker):
    ...
    for year in range(2019, 2024):
        ...
        weighted_sig = sum(
            weights[name] * sig.reindex(ret_yr.index).fillna(0).astype(float)
            for name, sig in signals_list
        )
...
def run_momentum_combos(prices_daily, daily_returns, tracker):
    ...
    for lookback in [20, 40]:
        for thresh in [0.0, 0.05, 0.10]:
            ...
            cfg_m  = {
                ...
                "lookback": lookback, "threshold": thresh,
    ...
    res_adx = walk_forward_static(
        prices_daily, daily_returns,
        lambda p: (momentum_signal(p, best_mom_lookback, best_mom_thresh) *
                   (compute_adx(p, 14) > 30).astype(int)).clip(0, 1),
        "mom_adx30",
    )
    ...
    cfg_ma = {
        ...
        "lookback": best_mom_lookback, "threshold": best_mom_thresh,
    ...
    res_va = walk_forward_static(
        prices_daily, daily_returns,
        lambda p: (momentum_signal(p, best_mom_lookback, best_mom_thresh) *
                   ((vol_regime(p, 20) == 1) & (compute_adx(p, 14) > 25)).astype(int)).clip(0, 1),
        "mom_vol_adx_AND",
    )
    ...
    res_dd = walk_forward_static(
        prices_daily, daily_returns,
        lambda p: make_mom_dd_sig(p, daily_returns, best_mom_lookback, best_mom_thresh,
                                   dd_threshold, dd_window),
        "mom_dd_filter",
    )
    ...
    cfg_dd = {
        ...
        "lookback": best_mom_lookback, "threshold": best_mom_thresh,
        "dd_threshold": dd_threshold, "dd_window": dd_window,
        "notes": (
...
@with_timeout(seconds=900)
def run_ml_meta_on_momentum(prices_daily, daily_returns, df_daily, avail10, avail20,
                             best_mom_thresh, best_mom_lookback, tracker):
    """
            ...
            cfg = {
                ...
                "notes": (
                    ...
                    + ("Better than Donchian meta-learner (0.843) — momentum has more learnable regime patterns."
                       if mean_sh > 0.843 else "Insufficient meta-signal — momentum regime patterns same as Donchian.")
                ),
            }
            metrics = {
                "sharpe": mean_sh,
...
def main():
    ...
    print("=" * 80)
    try:
        combos = run_lgbm_regime_combos(
            prices_daily, daily_returns, df_daily, target_daily, avail10, tracker
        )
    ...
    try:
        ml_res = run_ml_meta_on_momentum(
            prices_daily, daily_returns, df_daily, avail10, avail20,
            best_mom_thresh, best_mom_lb, tracker
        )
    ...
    for r in all_results.get("lgbm_combos", []):
        if isinstance(r, dict) and "mean" in r:
            ...
            print(f"  {r['name']:<53} {r['mean']:>8.3f} {r['std']:>8.3f} {sh22 if isinstance(sh22, str) else sh22:>8.3f}")
    ...
    if isinstance(ivw, dict) and "mean" in ivw:
        ...
        print(f"  {ivw['name']:<53} {ivw['mean']:>8.3f} {ivw['std']:>8.3f} {sh22 if isinstance(sh22, str) else sh22:>8.3f}")
    ...
    if isinstance(ml, dict) and "mean" in ml:
        ...
        print(f"  {ml['name']:<53} {ml['mean']:>8.3f} {ml['std']:>8.3f} {sh22 if isinstance(sh22, str) else sh22:>8.3f}")
    ...
    def safe_dict(d):
        if not isinstance(d, dict):
            return {}
        return {str(k): (float(v) if isinstance(v, (int, float, np.floating, np.integer)) else v)
                for k, v in d.items()}
