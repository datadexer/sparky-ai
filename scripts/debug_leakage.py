...
print(
    pd.DataFrame(
        {"next_open": next_open, "target_close": target_close, "target": (target_close > next_open).astype(int)}
    )
)
...
print("      (= (103 - 102) / 102 = 0.0098)")

# What is the target for this row?
print("\n  Target timing (should use FUTURE data only):")
print(f"    - Signal generated at: {test_date.date()} close")
print(
    f"    - Trade executed at: {(test_date + pd.Timedelta(days=1)).date()} open = {prices.loc[test_date + pd.Timedelta(days=1), 'open']}"
)
print(
    f"    - Target evaluated at: {(test_date + pd.Timedelta(days=2)).date()} close = {prices.loc[test_date + pd.Timedelta(days=2), 'close']}"
)
print(
    f"    - Target value: {1 if prices.loc[test_date + pd.Timedelta(days=2), 'close'] > prices.loc[test_date + pd.Timedelta(days=1), 'open'] else 0}"
)
print("      (105 > 104? → 1 = long)")
...
print(
    f"Target uses: open on {(test_date + pd.Timedelta(days=1)).date()} and close on {(test_date + pd.Timedelta(days=2)).date()}"
)
print("\nConclusion: ", end="")
...
for i in range(len(prices) - 3):
    ...
    if i > 0:  # Skip first row (returns_1d is NaN)
        ...
        print(f"\n{date_t.date()}:")
        print(
            f"  Feature uses: close[{prices.index[i - 1].date()}]={close_t_minus_1}, close[{date_t.date()}]={close_t}"
        )
        print(
            f"  Target uses: open[{prices.index[i + 1].date()}]={open_next}, close[{prices.index[i + 2].date() if i + 2 < len(prices) else 'N/A'}]={close_target}"
        )
