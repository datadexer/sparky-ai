...
def run_stage1(features_all, target, prices_daily, configs) -> list[dict]:
    ...
    logger.info(f"Train: {split_idx} rows ({features_all.index[0].date()} → {features_all.index[split_idx-1].date()})")
    logger.info(f"Test:  {len(features_all)-split_idx} rows ({features_all.index[split_idx].date()} → {features_all.index[-1].date()})\n")
...
def walkforward_config(cfg: dict, features_all, target, prices_daily) -> dict:
    ...
    for year in test_years:
        ...
        try:
            ...
            signals = signals_from_proba(y_proba, X_te.index)
            sharpe = compute_sharpe(signals, prices_daily)
            yearly.append({"year": year, "sharpe": sharpe, "acc": acc, "auc": auc,
...
def run_stage2(features_all, target, prices_daily, top_configs: list[dict], top_n: int = 5) -> list[dict]:
    ...
    for i, entry in enumerate(top_configs[:top_n], 1):
        cfg = entry["config"]
        logger.info(f"\n  [{i}/{top_n}] {cfg['family']} {cfg['features']} "
                    f"(screening Sharpe={entry['sharpe']:.3f})")
...
def main():
    ...
    for i, r in enumerate(stage1_results[:10], 1):
        cfg = r["config"]
        logger.info(f"  {i:2d}. {cfg['family']:10s} {cfg['features']:6s} "
    ...
    log_sweep_to_wandb(tracker, "stage1_screening", stage1_results, {
    ...
    log_sweep_to_wandb(tracker, "stage2_walkforward", validated, {
