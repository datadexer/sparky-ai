...
def compute_additional_features(df: pd.DataFrame, features_base: pd.DataFrame) -> pd.DataFrame:
    ...
    features["price_range_expansion"] = (df["high"] - df["low"]) / (
        df["high"].rolling(20).mean() - df["low"].rolling(20).mean()
    )
    ...
    features["volatility_momentum_interaction"] = features_base["realized_vol_24h"] * features_base["momentum_24h"]
    ...
    features["rsi_extreme_high_volume"] = ((features_base["rsi_14h"] > 70) & (df["volume"] > 2 * volume_ma_20)).astype(
        int
    )
    ...
    features["close_above_open_ratio_20h"] = (df["close"] > df["open"]).rolling(20).sum() / 20
    features["higher_close_streak"] = (df["close"] > df["close"].shift(1)).astype(int).rolling(10).sum()
    features["lower_close_streak"] = (df["close"] < df["close"].shift(1)).astype(int).rolling(10).sum()

    # === VOLUME PATTERN FEATURES (3 features) ===
    logger.info("Volume pattern features...")
    features["volume_trend_24h"] = df["volume"].rolling(24).mean() / df["volume"].rolling(168).mean()
    features["volume_spike_count_20h"] = (df["volume"] > 2 * volume_ma_20).rolling(20).sum()
    features["volume_dry_up"] = (df["volume"] < 0.5 * volume_ma_20).astype(int)
