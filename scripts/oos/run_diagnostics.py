...
try:
    ...
    if has_mvrv or has_nupl:
        panel += 1
        if has_mvrv:
            axes[panel].plot(onchain.loc[common_idx, "mvrv_zscore"], color=ORANGE, label="MVRV Z-Score", linewidth=1)
        if has_nupl:
...
dd_recovery_str = (
    f"{dd_recovery_idx.date()} ({(dd_recovery_idx - dd_trough_idx).days}d)" if dd_recovery_idx else "Not yet recovered"
)
