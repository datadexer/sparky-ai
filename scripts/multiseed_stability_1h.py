...
def split_by_timestamp(X: pd.DataFrame, y: pd.Series) -> Dict[str, Tuple[pd.DataFrame, pd.Series]]:
    ...
    for name, (X_split, y_split) in splits.items():
        print(f"{name:5s}: {len(X_split):6d} samples, "
              f"pos={y_split.sum()}/{len(y_split)} "
              f"({100*y_split.mean():.1f}%)")
...
def check_pass_criteria(stats: Dict, results: List[Dict]) -> Dict[str, bool]:
    ...
    print(f"Val AUC std < 0.005: {stats['val_auc_std']:.6f} -> "
          f"{'PASS' if val_auc_stable else 'FAIL'}")

    # Test AUC std < 0.01
    test_auc_stable = stats['test_auc_std'] < 0.01
    criteria['test_auc_stable'] = test_auc_stable
    print(f"Test AUC std < 0.01: {stats['test_auc_std']:.6f} -> "
          f"{'PASS' if test_auc_stable else 'FAIL'}")

    # Top-3 features consistent
    top3_consistent = all(count >= 3 for count in
                          list(stats['top3_feature_counts'].values())[:3])
    criteria['top3_consistent'] = top3_consistent
    print(f"Top-3 features appear in >=3 seeds: "
          f"{'PASS' if top3_consistent else 'FAIL'}")
    ...
    print(f"Mean val AUC ~{expected_auc} (±0.01): {stats['val_auc_mean']:.4f} -> "
          f"{'PASS' if val_auc_matches else 'FAIL'}")
...
def main():
    ...
    for seed in seeds:
        result = train_and_evaluate_seed(
