...
pytestmark = pytest.mark.skipif(
    not _has_onchain_data, reason="On-chain features data not available (requires local data files)"
)
...
def test_forward_fill_within_day(onchain_features):
    ...
    for col in day_data.columns:
        if day_data[col].notna().all():  # Skip if there are NaNs
            ...
            assert unique_values == 1, f"{col} has {unique_values} unique values within day {test_date}, expected 1"
...
def test_mvrv_zscore_warmup(onchain_features):
    ...
    assert time_diff < 5, f"MVRV Z-score first valid at {first_valid}, expected ~{expected_start} (30d min_periods)"
...
def test_alignment_with_btc_hourly(onchain_features, btc_hourly):
    ...
    assert len(onchain_features) == len(btc_hourly), "On-chain features and BTC hourly should have same length"
...
def test_no_duplicate_index(onchain_features):
    ...
    assert not onchain_features.index.duplicated().any(), "Found duplicate timestamps in index"


def test_monotonic_increasing_index(onchain_features):
    """Verify index is sorted in ascending order."""
    assert onchain_features.index.is_monotonic_increasing, "Index should be monotonically increasing"
...
def test_feature_correlation_sanity(onchain_features):
    ...
    if len(data) > 100:  # Need enough data points
        ...
        assert corr > 0.1, f"Active addresses and TX count changes should be positively correlated (got {corr:.3f})"
...
def test_data_coverage(onchain_features):
    ...
    for col in onchain_features.columns:
        ...
        assert coverage >= 0.60, f"{col} has only {coverage:.1%} coverage, expected >= 60%"
