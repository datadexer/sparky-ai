    ...
    steps = [
        Step(
            ...
            prompt=(
                ...
                "- Compute DSR analytically from summary stats (no return series needed):\n"
                "  ```python\n"
                "  from sparky.tracking.metrics import analytical_dsr, expected_max_sharpe\n"
                "  # analytical_dsr computes DSR from summary statistics directly.\n"
                "  # skewness/kurtosis default to Gaussian (0, 3) if not available.\n"
                "  # Kurtosis must be RAW Pearson (normal=3), NOT excess (normal=0).\n"
                "  T = 8760 * 5\n"
                "  dsr = analytical_dsr(\n"
                "      sr=best_sharpe,  # from summary files\n"
                "      skewness=None,   # None = Gaussian approx (may overstate DSR)\n"
                "      kurtosis=None,   # None = Gaussian approx\n"
                "      T=T,\n"
                "      n_trials=187,\n"
                "  )\n"
                "  ```\n"
